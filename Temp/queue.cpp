...
std::vector<Order> OrderQueue::getOrders() {
    loadOrders();  // Ensure the latest orders are loaded
    return orders;
}

bool OrderQueue::ClearOrder(int orderNumber) {
    auto it = std::remove_if(orders.begin(), orders.end(), [&](const Order &order) {
        return order.orderNumber == orderNumber;
    });
...
void OrderQueue::saveOrders(const std::string &filePath) {
    std::ofstream outFile(filePath, std::ios::trunc); // Open file in truncate mode to overwrite
    if (!outFile.is_open()) {
        std::cerr << "Error: Unable to open " << filePath << " for saving orders." << std::endl;
        return;
    }

    for (const auto &order : orders) {
        // Write order details to file
        outFile << order.orderNumber << ","
                << order.date << ","
                << order.orderName << ","
                << order.totalPrice << ",";

        // Write each item's details (ID, Name, Quantity)
        for (const auto &item : order.items) {
            outFile << std::get<0>(item) << ","    // Item ID
                    << std::get<1>(item) << ","    // Item Name
                    << std::get<2>(item) << ",";   // Quantity
        }

        // Add special request
        outFile << order.specialRequest << "\n";
    }

    outFile.close();
    std::cout << "Orders saved to " << filePath << std::endl;
}
